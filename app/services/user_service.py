...
class UserService:
    ...
    async def login(self, email: str, password: str) -> tuple[str, str]:
        ...
        await self.repository.set_refresh_token(user, refresh_token)
        return access_token, refresh_token

    # NFR-USER-001 액세스 토큰 재발급 (서버에 저장된 리프레시 토큰과 일치할 때만 재발급)
    async def reissue_access_token(self, user_id: int, refresh_token: str) -> str:
        user = await self.repository.get_by_id(user_id)
        if (
            not user
            or not user.is_active
            or user.current_refresh_token != refresh_token
        ):
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED,
    ...
    # REQ-USER-003 로그아웃 (서버에 저장된 리프레시 토큰 무효화)
    async def logout(self, user: User) -> None:
        await self.repository.set_refresh_token(user, None)
